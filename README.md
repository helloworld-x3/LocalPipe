...
| 行为测试 | **161 项通过** | [`test_pipeline.py`](test_pipeline.py)，不调用真实 LLM；CI 在 Python 3.12 运行 |
...
飞书不是单纯的触发器或结果展示页，而是连接品牌方、创意团队、市场审核人与文化知识库的人机协同底座。当前 HTTP 自动化原型把任务状态从“待生成”推进到“生成中/待审核”，写回三候选、系统推荐、分数、排名、理由、审核策略、不确定性以及每条候选的 KreadoAI Prompt/JSON；同时建立候选明细、人工审核、规则修订、事件台账和指标表。人工在飞书完成采纳、修改、风险确认和任务关闭，反馈再进入 AI 归纳与人工批准的修订候选，生成和审核流转耗时由系统自动记录。
