...
## 2026-09-01｜阿里云部署与飞书云端闭环验收

### 部署状态

- 阿里云 Ubuntu 24.04 服务器已完成 LocalPipe 部署。
- systemd 服务 `localpipe` 已启用开机自启和异常自动重启。
- Python 服务只监听 `127.0.0.1:8080`，不直接向公网开放 8080 端口。
- Nginx 作为公网反向代理，已启用 Let's Encrypt HTTPS 和 HTTP 到 HTTPS 跳转。
- 当前演示入口：`https://localpipe.47-250-193-225.sslip.io`。
- `/health` 公网检查返回 HTTP 200；未携带有效认证 Header 的 `/trigger` 返回 HTTP 401。
- 飞书自动化“任务待生成触发 LocalPipe”已从临时 Cloudflare 隧道切换到固定 HTTPS `/trigger` 地址，并保存启用。

### 端到端受控验收

测试任务：`CLOUD-FR-0901-01`，法国 Meta 服饰样板。
...
- 写入 1 条结果记录；
- 写入 1 条人工审核记录；
- 记录 `queued` 和 `completed` 运行事件；
- AI 生成耗时约 45.372 秒；
- 自动化端到端完成耗时约 75.592 秒；
- 本次禁忌风险等级为低风险。

### 本次发现的问题与边界

- 本次 3 个候选均因 `fidelity_structure_invalid` 被硬门禁拦截，系统按既有规则输出 `review_policy=block`、高不确定性和“无可发布候选”，没有强行推荐。
- 初步定位为任务中的整段“品牌要求”被作为单个 `protected_term` 进入结构匹配，而模型回检使用了另一种保护词表述，导致结构误判；需单独修复并回归测试。
- 人工审核表已成功创建记录，但飞书任务节点未创建，因为该测试任务未填写审核负责人；后续需配置默认审核负责人或要求任务创建时必填。
- 本次验收证明云端触发、生成、质检、三候选回写和审核表闭环可运行，不代表已经完成长期 SLA 验证，也不代表企业真实 ROI、投放效果或 KreadoAI 正式 API 接入。
- 服务器 `.env`、模型密钥、飞书应用 Secret 和自动化 Token 均不纳入 Git 仓库。
